...
@IonicPage()
@Component({
  selector: 'page-jobcalendar',
  templateUrl: 'jobcalendar.html',
...
active_job_breadcrumb = localStorage.getItem('active_job_breadcrumb');
  constructor(public modalCtrl: ModalController, public navCtrl: NavController, public navParams: NavParams, public companyProvider: CompanyProvider, public alertCtrl: AlertController, public toastCtrl: ToastController,public loadingCtrl: LoadingController, private viewCtrl:ViewController) {
    ...
    this.getAllYears();
    this.companyProvider.allTrades(this.job_id).subscribe((data)=>{
      this.trades = data;
    },
    err => {
  ...
  {
  	this.companyProvider.getCalendarEvents(this.job_id,'jobs').subscribe((all_events)=>{
  ...
  add_new_event(myEvent14) 
   {
    let modal = this.modalCtrl.create('AddeventPage', { current_date: this.currentDate, isAdd : '1', isTrade: '1', trades: this.trades, all_events: this.all_events });
    modal.onDidDismiss(data => {
      if(data != undefined)
      {
        ...
        this.companyProvider.addSingleCalendarEvent(this.job_id,'0',data).subscribe((formdata)=>{
        	var eventMsg = '1';
          loading.dismissAll();
  ...
  onViewTitleChanged(title) {
    ...
    {
    	let modal = this.modalCtrl.create('AddeventPage', { event_id: event_id, isEdit : '1', eve : event, isTrade: '1', trades: this.trades, all_events: this.all_events });
	    modal.onDidDismiss(data => {
	      if(data != undefined)
	      {
          const loading = this.loadingCtrl.create({});
